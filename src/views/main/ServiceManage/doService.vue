<template>
    <div>
        <div style="box-shadow: 0 2px 12px 0 rgba(0,0,0,0.1);padding-left: 1rem;">
            <el-descriptions title="服务处理">
                <el-descriptions-item label="对于客户提出的咨询、建议、投诉等服务进行处理，并记录下处理结果"></el-descriptions-item>
            </el-descriptions>
        </div>
    ...
    margin-right: 20px;">
                ...
                <template #operate2="{ row }">
                    <vxe-button size="mini" content="处理" status="warning" round v-if="row.svrStatus === '已分配'"
                        @click="toDo(row)">
                    </vxe-button>
                    <vxe-modal v-model="modalStatus" show-footer :draggable="false" width="50%" title="服务信息（状态：已分配）"
                        @confirm="doForm()">
                        <template #default>
                            <vxe-form :data="form" title-align="right" custom-layout>
                                ...
                                <vxe-form-item title="服务处理 :" :span=12>
                                    <template #default>
                                        <el-input v-model="form.svrDeal" :rows="2" type="textarea"
                                            placeholder="请输入服务处理" />
                                    </template>
                                </vxe-form-item>
                            </vxe-form>
...
import { serviceByLike, editService } from '@/api/service'
import { ElMessage } from 'element-plus'

var modalStatus = ref(false)
var form = reactive(
    { svrId: '', svrDeal: '', svrCustName: '', svrType: '', svrTitle: '', svrRequest: '', svrCreateBy: '', svrCreateDate: '', svrDueTo: '', svrDueDate: '' }
)
...
const gridOptions = reactive<VxeGridProps>({
    ...
    columns: [
        ...
        { field: 'svrDueTo', title: '分配人' },
        { field: 'svrDueDate', title: '分配时间' },
        { title: '处理', width: 100, slots: { default: 'operate2' } }
    ],
    proxyConfig: {
        form: true,
